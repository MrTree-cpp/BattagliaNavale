...
Implementazione Client-Server del gioco Battaglia Navale (Battleship) in Java, realizzata da Pavel Cavallini e Mirko Pratesi.
...
Avvia il server (terminale 1): mvn exec:java -Dexec.mainClass="Server.ServerMain"

Il server apre la porta 6767 e aspetta il client. Nota: Maven scaricherà ed eseguirà il plugin exec se non è già presente.

Avvia il client (terminale 2): mvn exec:java -Dexec.mainClass="Client.ClientMain"

Il client si connetterà al server sulla porta 6767 (impostazione hardcoded) e inizierà il loop di gioco.

Se preferisci eseguire direttamente le classi compilate (senza plugin exec), dopo mvn package puoi avviare con: java -cp target/classes Server.ServerMain java -cp target/classes Client.ClientMain

Protocollo di rete (Common/Protocollo.java) Il protocollo è testuale, con messaggi separati dal carattere ":".
...
MISS — acqua
SUNK — affondato (formato: "SUNK:", es. "SUNK:P") dove è un identificatore di nave (P, C, I, U, D, S)
Separatore
...
Dopo un colpo, se non esistono più celle intatte della stessa ID, la nave è considerata affondata; BotHandler risponde "SUNK:" e marca quei pezzi come affondati nella board.
...
Apri due terminali (server e client).
Avvia Server.ServerMain (vedi comandi sopra).
Avvia Client.ClientMain — segui le istruzioni a video per piazzare le navi (manuale o random) e spara quando ricevi TURN_START.
Ripeti fino a GAMEOVER per uno dei giocatori.


Repository originale: MrTree-CP/BattleShip
